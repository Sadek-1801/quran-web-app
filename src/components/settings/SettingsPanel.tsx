...
export function SettingsPanel() {
  ...
  return (
    <div className="p-4 space-y-6">
      {/* Dark Mode — visible only on small screens (hidden on md+, where it's in navbar) */}
      <div className="md:hidden">
        <label className="block text-sm font-medium text-foreground/70 mb-2">Appearance</label>
        <button
          onClick={() => updateSetting("darkMode", !settings.darkMode)}
          className={`cursor-pointer w-full flex items-center gap-2 rounded-md border px-3 py-2 text-sm transition-colors ${
            settings.darkMode
        ...
        >
          ...
          <span className="font-medium">{settings.darkMode ? "Dark Mode" : "Light Mode"}</span>
        </button>
      </div>

      {/* Arabic Font */}
      <div>
        <label className="block text-sm font-medium text-foreground/70 mb-2">Arabic Font</label>
        <div className="space-y-2">
          {FONT_OPTIONS.map((opt) => (
            ...
            >
              ...
              <span className="text-foreground/40 ml-1">&mdash; {opt.description}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div>
        <label className="block text-sm font-medium text-foreground/70 mb-2">Translation Language</label>
        <div className="space-y-2">
          {TRANSLATION_OPTIONS.map((opt) => (
            ...
            >
              <span className="font-medium">{opt.label}</span>
              <span className="text-foreground/40 ml-1">&mdash; {opt.source}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="rounded-md border border-border p-3">
        <p className="text-xs text-foreground/40 mb-2">Preview</p>
        <p
